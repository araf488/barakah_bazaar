...
**Status:** planned — Phase 1 (transactional), Phase 5 (full)

SMS, push and email delivery with a log of what was sent.
...
- All sends are queued (BullMQ) with retries. A failed SMS must never fail a checkout.
- SMS goes through a local Bangladesh gateway (Alpha SMS / SSL Wireless) via the SmsGateway port in the Auth module.
- Push is Firebase Cloud Messaging, shared by the Flutter app and the web storefront.
- Log the template id and recipient, never the rendered body — bodies carry OTPs.
